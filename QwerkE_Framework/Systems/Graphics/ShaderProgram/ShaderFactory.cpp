...
ShaderProgram* ShaderFactory::CreateShader(eShaderTypes type)
{
	...
	shader->SetShaderStringData(GL_VERTEX_SHADER, CreateVertexShader(type));
	shader->SetShaderStringData(GL_FRAGMENT_SHADER, CreateFragmentShader(type));
	shader->SetShaderStringData(GL_GEOMETRY_SHADER, CreateGeometryShader(type));
	shader->Init(NULL, NULL, NULL);
	return shader;
}
